...
namespace WebArticleURLToText
{
    public class NewsPage
    {
        ...
        {
            InitializeWebsite();
            doc = GetWebsite(webaddress);      // Initialization
            GetArticleHead();
            GetArticleBody();
        }
        ...
        private HtmlDocument GetWebsite(string webaddress)  // Method that downloads the string from website.
        {
            ...
            else
            {
                ...
                string websiteString = new WebClient().DownloadString(webaddress); //WebException
        ...
        private void GetArticleHead()   // Methode: Gets the article title.
        {
        ...
        private HtmlNode GetArticleNode()    // Methode that gets article nods - (A node is any piece of individual content)
        {
            ...
            {
                Tuple<string, string> vals = websites[domain]; //KeyNotFoundException
